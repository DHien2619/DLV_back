...
const momentEmoji = {
  ...
  question_unanswered: '?', price_mention: '$', competitor_mention: 'vs', emotional_peak: '💥'
};
const phaseLabel = {
  ...
  if (value) {
    return (
      <div className="ca-picked">
        <span><IconCustomer size={14}/> <b>{value.name}</b> {value.phone && `· ${value.phone}`}</span>
      ...
      <div className="ca-home">
        ...
        <div className="ca-brand-strip">
          ...
          <div className="ca-brand-right">
            <Link to="/customers" className="ca-brand-link"><IconCustomers size={16}/> Khách hàng</Link>
          </div>
        </div>
      ...
      <div className="ca-root">
        <div className="ca-loading">
          ...
          <p className="ca-loading-steps">
            Diarize transcript <IconArrowRight size={14}/> Quality Rubric <IconArrowRight size={14}/> Needs <IconArrowRight size={14}/> Opportunity <IconArrowRight size={14}/> Compliance <IconArrowRight size={14}/> Structure
          </p>
        </div>
      </div>
    );
    ...
    <div className="ca-root ca-result">
      ...
      <div className="ca-topbar">
        <div>
          ...
          <small>
            ...
            {analysis.rag_context_used && <span className="ca-rag-badge"> <IconMemory size={16}/> RAG context active</span>}
            {analysis.customer_id && <span className="ca-customer-badge"> · Linked to KH</span>}
          </small>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <Link to="/customers" className="ca-btn-ghost"><IconCustomers size={16}/> Customers</Link>
          <button className="ca-btn-ghost" onClick={() => { setAnalysis(null); setFile(null); setAudioUrl(null); setCustomer(null); setShowMatches(false); }}>
            <IconArrowLeft size={14}/> Cuộc gọi khác
          </button>
        </div>
      </div>
      ...
      {showMatches && analysis.match_candidates?.candidates?.length > 0 && (
        <div className="ca-match-banner">
          <div>
            <b><IconSearch size={16}/> Agent gợi ý {analysis.match_candidates.candidates.length} KH khớp:</b>
      ...
      <div className="ca-grid">
            ...
            {diarized?.segments?.map((s, i) => (
              ...
              >
                <div className="ca-seg-head">
                  <span className="ca-seg-speaker">{s.speaker === 'AGENT' ? '🏥 Agent' : s.speaker === 'CUSTOMER' ? '👤 KH' : '?'}</span>
        ...
        <div className="ca-col ca-col-insights">
          ...
          {quality && !quality.error && (
            <section className="ca-card">
              <div className="ca-card-head">
                <h3><IconChartLine size={16}/> Chất lượng tư vấn</h3>
                    ...
                    <div key={k} className="ca-rubric-row">
                      ...
                      {x.evidence?.[0] && (
                        <button className="ca-evidence" onClick={() => jumpTo(x.evidence[0].timestamp_sec)}>
                          <IconExternal size={14}/> {fmtTime(x.evidence[0].timestamp_sec)} · "{x.evidence[0].quote.slice(0, 70)}{x.evidence[0].quote.length > 70 ? '…' : ''}"
                        </button>
                      )}
                    </div>
                  );
              ...
              <div className="ca-strengths">
                <div>
                  <h4><IconCheck size={14}/> Điểm mạnh</h4>
                  <ul>{quality.top_strengths?.map((s, i) => <li key={i}>{s}</li>)}</ul>
                </div>
                <div>
                  <h4><IconTarget size={16}/> Cần cải thiện</h4>
          ...
          {opportunity && !opportunity.error && (
            <section className="ca-card">
              <div className="ca-card-head">
                <h3><IconMoney size={16}/> Cơ hội bán hàng</h3>
                ...
                <div>
                  ...
                  {opportunity.buying_signals.map((s, i) => (
                    <button key={i} className="ca-chip ca-chip-signal" onClick={() => jumpTo(s.timestamp_sec)}>
                      <IconMoney size={16}/> {fmtTime(s.timestamp_sec)} [{s.strength}] {s.signal_type} — "{s.quote.slice(0, 50)}…"
                    </button>
                  ))}
                </div>
              )}
          ...
          {compliance && !compliance.error && (
            <section className="ca-card">
              <div className="ca-card-head">
                <h3><IconWarning size={14}/> Compliance</h3>
              ...
              {compliance.events?.length === 0 ? (
                <p className="ca-clean"><IconCheck size={14}/> Không phát hiện vi phạm tuân thủ.</p>
              ) : (
          ...
          {needs && !needs.error && (
            <section className="ca-card">
              <h3><IconMemory size={16}/> Nhu cầu khách hàng</h3>
              {needs.medical_conditions?.length > 0 && (
                <div className="ca-needs-block">
                  <h4><IconStethoscope size={14}/> Tình trạng sức khỏe</h4>
              ...
              {needs.current_medications?.length > 0 && (
                <div className="ca-needs-block">
                  <h4><IconPill size={14}/> Thuốc đang dùng</h4>
              ...
              {needs.decision_style && (
                <div className="ca-needs-block">
                  <h4><IconCompass size={14}/> Phong cách quyết định</h4>
                  <p>{needs.decision_style}</p>
                </div>
              )}
              {needs.family_context && (
                <div className="ca-needs-block">
                  <h4><IconHandshake size={14}/> Bối cảnh gia đình</h4>
        ...
        <div className="ca-col ca-col-timeline">
          {structure && !structure.error && (
            <>
              <section className="ca-card">
                <h3><IconPhone2 size={16}/> Talk ratio</h3>
              ...
              {structure.phases?.length > 0 && (
                <section className="ca-card">
                  <h3><IconLayers size={16}/> Các giai đoạn</h3>
                  <div className="ca-phases">
                    {structure.phases.map((p, i) => (
                      <div key={i} className="ca-phase" onClick={() => jumpTo(p.start_sec)}>
                        ...
                        <p>{p.summary}</p>
                        {p.quality_note && <small><IconLightbulb size={14}/> {p.quality_note}</small>}
                      </div>
                    ))}
                  </div>
                </section>
              ...
              {structure.moments?.length > 0 && (
                <section className="ca-card">
                  <h3><IconClock size={14}/> Moments</h3>
